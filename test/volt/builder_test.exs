defmodule Volt.BuilderTest do
  ...
  describe "build/1" do
    ...
    test "CSS @import url assets resolve from imported files" do
      ...
      File.write!(
        Path.join(@fixture_dir, "src/imported_url_app.css"),
        "@import './components/button.css';"
      )

      {:ok, result} =
        Volt.Builder.build(
          entry: Path.join(@fixture_dir, "src/imported_url_app.css"),
          outdir: @outdir,
          minify: false,
          sourcemap: false
        )
